...
class Frontend
{
    ...
    public function enqueueStyles(): void
    {
        ...
        $styleFileName = ($this->settings->getDebug() === true) ? 'ups-tracking-frontend.css' : 'ups-tracking-frontend.min.css';
    ...
    public function enqueueScripts(): void
    {
        ...
        $scripFileName = ($this->settings->getDebug() === true) ? 'ups-tracking-frontend.js' : 'ups-tracking-frontend.min.js';
        ...
        {
            exit(esc_html__('Script could not be registered: ', 'ups-tracking') . $scriptUrl);
        }
         ...
         */
        $contactFormScripFileName = ($this->settings->getDebug() === true) ? 'ups-tracking-frontend.js' : 'ups-tracking-frontend.min.js';
        $contactFormScriptUrl = plugin_dir_url(__FILE__) . 'js/' . $contactFormScripFileName;
        if (wp_register_script($this->pluginSlug . 'form', $contactFormScriptUrl, array('jquery'), $this->version, false) === false)
        {
            exit(esc_html__('Script could not be registered: ', 'ups-tracking') . $contactFormScriptUrl);
        }
    }
}
